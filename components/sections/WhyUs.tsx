  ...
  return <section id="why" className="mesh-bone border-t border-rule">
    <div className="container-x py-24 max-[767px]:py-16">
      ...
      <div className="grid grid-cols-3 gap-5 max-[980px]:grid-cols-2 max-[640px]:grid-cols-1">
        ...
        {closing && <article className="mesh-dark grid items-center gap-10 rounded-panel p-9 text-white min-[981px]:col-span-3 min-[981px]:grid-cols-[1fr_auto] max-[980px]:col-span-2 max-[640px]:col-span-1 max-[560px]:p-7">
          <div><span className="font-mono text-[11px] max-[1199px]:text-[12px] uppercase tracking-[.14em] text-gold">{closing.title}</span><p className="mb-0 mt-3 max-w-[42ch] font-display text-[clamp(21px,2.1vw,28px)] italic leading-snug">{closing.description}</p></div>
