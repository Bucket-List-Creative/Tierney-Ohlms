  ...
  return <section id="services" className="mesh-dark border-t border-white/10 text-white">
    <div className="container-x py-24 max-[767px]:py-16">
      ...
      <div className="grid grid-cols-[1.05fr_.95fr] items-start gap-16 max-[980px]:grid-cols-1">
        <div>
          {services.map((service,i) => <Link key={service._id} href={`/services/${service.slug}`} onMouseEnter={()=>setActive(i)} onFocus={()=>setActive(i)} className={`group grid grid-cols-[44px_1fr_28px] items-center gap-4 border-t border-white/12 px-2 py-5 text-white transition-all duration-300 hover:pl-3 ${active===i?"pl-3":""}`}>
        ...
        {current && <aside className="sticky top-[calc(var(--header-h)+24px)] flex min-h-[320px] flex-col gap-[18px] rounded-panel border border-white/14 bg-white/[.04] p-8 max-[980px]:hidden">
          <span className="font-mono text-[11px] max-[767px]:text-[12px] uppercase tracking-[.14em] text-gold">Service {String(active+1).padStart(2,"0")} / {String(services.length).padStart(2,"0")}</span>
