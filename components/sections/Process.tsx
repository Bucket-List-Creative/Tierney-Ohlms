...
export function Process({ header, steps }: { header: HeaderData; steps: ProcessStep[] }) {
  ...
  return <section ref={runway} id="process" className="relative h-[340vh] border-t border-rule bg-white">
    <div className="sticky top-[var(--header-h)] flex h-[calc(100svh-var(--header-h))] flex-col justify-center overflow-hidden py-8">
      <header className="container-x mb-9 flex w-full flex-col items-start gap-3 text-left"><span className="eyebrow">{header.eyebrow}</span><h2 className="m-0 font-display text-[clamp(28px,3vw,40px)] font-medium leading-[1.14]">A clear path from <em className="gradient-text">day one.</em></h2><p className="m-0 max-w-[600px] text-[15.5px] leading-relaxed text-slate">{header.lead}</p></header>
